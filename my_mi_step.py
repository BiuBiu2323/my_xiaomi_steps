# -*- coding: UTF-8 -*-
"""
小米运动 / Zepp Life 步数修改脚本
- 支持 GitHub Actions / 本地 / 阿里云函数计算 三种运行方式
- 通过环境变量 MI_USER / MI_PASSWD 读取账号（多账号用 # 分隔）
"""

import requests, os, re, time, json, sys, tempfile
from random import randint
from urllib.parse import urlencode


# ======================== 默认配置 ========================
DEFAULT_USER = ""
DEFAULT_PASSWORD = ""
DEFAULT_MIN_STEPS = 17760
DEFAULT_MAX_STEPS = 23659

# Token 缓存文件（跨平台：Windows/Linux/云函数均可写）
TOKEN_FILE = os.path.join(tempfile.gettempdir(), "token.txt")

# Server酱 SendKey（微信推送通知，也支持环境变量 MI_SENDKEY）
SERVER_CHAN_SENDKEY = ""


# ======================== 带重试的请求 ========================
def _request(method, url, retries=3, **kwargs):
    """网络请求重试封装，应对偶发的 SSL/超时抖动"""
    last_err = None
    for i in range(retries):
        try:
            return requests.request(method, url, timeout=15, **kwargs)
        except Exception as e:
            last_err = e
            if i < retries - 1:
                time.sleep(2 * (i + 1))
    print(f"------ 请求失败 {url}: {last_err} ------")
    return None


def _safe_json(resp):
    try:
        return resp.json()
    except Exception:
        return None


# ======================== Token 缓存 ========================
def logToken(lt):
    try:
        with open(TOKEN_FILE, "w", encoding="utf-8") as f:
            f.write(lt)
    except Exception:
        pass  # 写入失败不阻塞流程


def readToken():
    try:
        if os.path.isfile(TOKEN_FILE):
            with open(TOKEN_FILE, "r", encoding="utf-8") as f:
                lt = f.read().strip()
                if lt:
                    return lt
    except Exception:
        pass
    return False


# ======================== 登录 ========================
def login(uu, pwd):
    phone = re.match(r"^1\d{10}$", uu)
    name = f"+86{uu}" if phone else uu
    third_name = "huami_phone" if phone else "huami"

    headers = {
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "user-agent": "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)",
        "app_name": "com.xiaomi.hm.health",
    }

    # 阶段1：获取 access code（已实测 api-user.zepp.com 旧路径可用）
    url1 = "https://api-user.zepp.com/registrations/" + name + "/tokens"
    data1 = urlencode({
        "client_id": "HuaMi",
        "country_code": "CN",
        "json_response": "true",
        "name": name,
        "password": pwd,
        "redirect_uri": "https://s3-us-west-2.amazonaws.com/hm-registration/successsignin.html",
        "state": "REDIRECTION",
        "token": "access",
    })
    res1 = _request("POST", url1, data=data1, headers=headers)
    if res1 is None:
        return None

    if res1.status_code == 429:
        print("------ 请求过于频繁，请变更IP或稍后再试 ------")
        return None

    j1 = _safe_json(res1)
    code = None
    if j1 and "access" in j1:
        code = j1["access"]
    elif j1 and j1.get("error"):
        print(f"------ 用户名或密码错误 (server error={j1.get('error')})，请检查后重试 ------")
        return None
    else:
        print(f"------ Access Code：HTTP {res1.status_code}，账号登录失败 ------")
        return None

    # 阶段2：code 换 login_token / app_token（zepp 优先，huami 兜底）
    data2 = urlencode({
        "app_name": "com.xiaomi.hm.health",
        "country_code": "CN",
        "code": code,
        "device_id": "02:00:00:00:00:00",
        "device_model": "android_phone",
        "app_version": "6.12.0",
        "grant_type": "access_token",
        "allow_registration": "false",
        "source": "com.xiaomi.hm.health",
        "third_name": third_name,
    })
    for url2 in ("https://account.zepp.com/v2/client/login",
                 "https://account.huami.com/v2/client/login"):
        res2 = _request("POST", url2, data=data2, headers=headers)
        if res2 is None:
            continue
        j2 = _safe_json(res2)
        token_info = (j2 or {}).get("token_info")
        if res2.status_code == 200 and token_info and token_info.get("login_token"):
            logToken(token_info["login_token"])
            return {
                "user_id": token_info.get("user_id"),
                "app_token": token_info.get("app_token"),
            }
        print(f"------ Login Code：HTTP {res2.status_code} @ {url2}，尝试下一域名 ------")

    print("------ 账号登录失败（两个域名均未成功） ------")
    return None


def getToken(lt):
    headers = {
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "user-agent": "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)",
        "app_name": "com.xiaomi.hm.health",
    }

    # Token 续期
    res1 = _request("GET", f"https://account.zepp.com/v1/client/renew_login_token?login_token={lt}",
                    headers=headers)
    if res1 is not None:
        j1 = _safe_json(res1)
        if j1 and "token_info" in j1:
            logToken(j1["token_info"]["login_token"])

    # 获取 app_token
    res2 = _request("GET", f"https://account.zepp.com/v1/client/app_tokens?login_token={lt}",
                    headers=headers)
    if res2 is not None:
        j2 = _safe_json(res2)
        if j2 and "token_info" in j2:
            return {
                "user_id": j2["token_info"].get("user_id"),
                "app_token": j2["token_info"].get("app_token"),
            }

    print("------ Token 已失效，需要重新登录 ------")
    return None


# ======================== 时间戳 ========================
def get_time():
    try:
        url = "https://acs.m.taobao.com/gw/mtop.common.getTimestamp/"
        res = _request("GET", url, headers={
            "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            "User-Agent": "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)",
        })
        if res is not None:
            t = res.json()["data"]["t"]
            return t
    except Exception as e:
        print(f"------ 网络时间戳获取失败：{e}，使用本地时间 ------")
    return str(int(time.time() * 1000))


def beijing_today():
    """按东八区计算日期，避免境外服务器 UTC 日期差一天"""
    return time.strftime("%F", time.gmtime(time.time() + 8 * 3600))


# ======================== 消息推送 ========================
def send_server_chan(title, content, sendkey=None):
    if sendkey is None:
        sendkey = os.environ.get("MI_SENDKEY") or SERVER_CHAN_SENDKEY
    if not sendkey:
        return
    try:
        r = _request("POST", f"https://sctapi.ftqq.com/{sendkey}.send",
                     data={"title": title, "desp": content})
        resp = _safe_json(r) if r is not None else None
        if resp and resp.get("code") == 0:
            print(f"Server酱推送成功: {title}")
        else:
            print(f"Server酱推送失败: {resp}")
    except Exception as e:
        print(f"Server酱推送异常: {e}")


def send_telegram(title, content):
    bot_token = os.environ.get("XM_TG_BOT_TOKEN")
    chat_id = os.environ.get("XM_TG_USER_ID")
    if not bot_token or not chat_id:
        return
    try:
        r = _request("POST", f"https://api.telegram.org/bot{bot_token}/sendMessage",
                     data={"chat_id": chat_id, "text": f"{title}\n\n{content}"})
        if r is not None and r.status_code == 200:
            print("Telegram推送成功")
        else:
            print(f"Telegram推送失败: {r.text[:120] if r is not None else 'no response'}")
    except Exception as e:
        print(f"Telegram推送异常: {e}")


def push(title, content):
    send_server_chan(title, content)
    send_telegram(title, content)


# ======================== 单账号刷步 ========================
def run_one(uu, pwd, min_steps, max_steps):
    """执行单个账号，返回 (是否成功, 结果描述)"""
    token = None
    relog = readToken()

    if relog is False:
        token = login(uu, pwd)
    else:
        token = getToken(relog)
        if token is None:
            token = login(uu, pwd)

    if token is None or not token.get("app_token"):
        msg = "------ 登录失败，步数未提交，请检查账号密码或网络 ------"
        print(msg)
        return False, msg

    timestamp = get_time()
    dateToday = beijing_today()
    if min_steps > max_steps:
        min_steps, max_steps = max_steps, min_steps
    steps = str(randint(min_steps, max_steps))

    # 完整步数数据模板（data_hr / stage 等字段），仅替换日期与总步数
    data_json = DATA_JSON_TEMPLATE.replace('2025-08-17', dateToday).replace('17760', steps)

    headers0 = {
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "user-agent": "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)",
        "app_name": "com.xiaomi.hm.health",
        "apptoken": token["app_token"],
    }
    url0 = f"https://api-mifit-cn.huami.com/v1/data/band_data.json?t={timestamp}"
    data0 = f"userid={token['user_id']}&last_sync_data_time=1628256960&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={data_json}"
    res0 = _request("POST", url0, data=data0, headers=headers0, retries=3)

    if res0 is None:
        msg = f"账号 {uu}：步数提交请求失败（网络原因），步数目标 {steps}"
        push("步数刷新失败", msg)
        return False, msg

    j0 = _safe_json(res0)
    if j0 is None:
        msg = f"账号 {uu}：服务器返回非JSON（HTTP {res0.status_code}）：{res0.text[:200]}"
        push("步数刷新失败", msg)
        return False, msg

    step_code = j0.get("code", -1)
    step_msg = j0.get("message", "unknown")
    ok = (step_msg == "success") or (step_code in (0, 1))

    if ok:
        push_title = f"步数刷新成功：{steps} 步"
        status = "刷新成功"
    else:
        push_title = "步数刷新失败"
        status = f"刷新失败 (code={step_code})"

    push_content = f"账号：{uu}\n今日步数：{steps}\n状态：{status}\n服务器返回：{json.dumps(j0, ensure_ascii=False)}"
    push(push_title, push_content)

    result = f"账号：{uu}\n步数：{steps}\n{res0.text[:300]}\n"
    print(result)
    return ok, result


# ======================== 多账号调度 ========================
def run_accounts(user_str, pwd_str, min_steps, max_steps):
    users = [x.strip() for x in str(user_str).split("#") if x.strip()]
    pwds = [x.strip() for x in str(pwd_str).split("#") if x.strip()]

    if not users:
        return False, "未配置账号：请设置环境变量 MI_USER / MI_PASSWD"

    if len(pwds) == 1 and len(users) > 1:
        pwds = pwds * len(users)
    if len(pwds) != len(users):
        return False, f"账号数({len(users)})与密码数({len(pwds)})不匹配，请检查 MI_USER / MI_PASSWD"

    all_ok = True
    results = []
    for i, (u, p) in enumerate(zip(users, pwds)):
        if i > 0:
            time.sleep(5)  # 多账号间隔，避免触发限流
        ok, msg = run_one(u, p, min_steps, max_steps)
        results.append(msg)
        if not ok:
            all_ok = False

    summary = "\n".join(results)
    return all_ok, summary


def main(uu, pwd, min_steps=None, max_steps=None):
    """入口：兼容阿里云函数计算 main_handler 直接调用，返回结果字符串"""
    min_steps = DEFAULT_MIN_STEPS if min_steps is None else int(min_steps)
    max_steps = DEFAULT_MAX_STEPS if max_steps is None else int(max_steps)
    ok, summary = run_accounts(uu, pwd, min_steps, max_steps)
    return summary


# ======================== 阿里云函数计算入口 ========================
def main_handler(event, context):
    """
    阿里云函数计算 (FC) 入口函数。
    event 参数示例：
    {"user": "xx@qq.com", "password": "xxx", "minSteps": 17760, "maxSteps": 23659}
    也支持环境变量：MI_USER / MI_PASSWD / MI_MIN_STEPS / MI_MAX_STEPS
    """
    def pick(*vals):
        for v in vals:
            if v:
                return v
        return None

    if isinstance(event, dict):
        user = pick(event.get("user"), os.environ.get("MI_USER"), DEFAULT_USER)
        password = pick(event.get("password"), os.environ.get("MI_PASSWD"),
                        os.environ.get("MI_PASSWORD"), DEFAULT_PASSWORD)
        min_steps = pick(event.get("minSteps"), os.environ.get("MI_MIN_STEPS"))
        max_steps = pick(event.get("maxSteps"), os.environ.get("MI_MAX_STEPS"))
    elif isinstance(event, str) and event:
        try:
            params = json.loads(event)
            user = pick(params.get("user"), os.environ.get("MI_USER"), DEFAULT_USER)
            password = pick(params.get("password"), os.environ.get("MI_PASSWD"),
                            os.environ.get("MI_PASSWORD"), DEFAULT_PASSWORD)
            min_steps = pick(params.get("minSteps"), os.environ.get("MI_MIN_STEPS"))
            max_steps = pick(params.get("maxSteps"), os.environ.get("MI_MAX_STEPS"))
        except Exception:
            user, password, min_steps, max_steps = None, None, None, None
    else:
        user, password, min_steps, max_steps = None, None, None, None

    if not user:
        user = DEFAULT_USER
    if not password:
        password = DEFAULT_PASSWORD

    def to_int(v, default):
        try:
            return int(v)
        except (TypeError, ValueError):
            return default

    min_steps = to_int(min_steps, DEFAULT_MIN_STEPS)
    max_steps = to_int(max_steps, DEFAULT_MAX_STEPS)

    return main(user, password, min_steps, max_steps)


# ======================== 本地 / GitHub Actions 运行入口 ========================
def _env_int(names, default):
    for n in names:
        v = os.environ.get(n)
        if v:
            try:
                return int(v)
            except ValueError:
                print(f"环境变量 {n}={v!r} 不是数字，忽略")
    return default


if __name__ == "__main__":
    user = os.environ.get("MI_USER") or DEFAULT_USER
    password = os.environ.get("MI_PASSWD") or os.environ.get("MI_PASSWORD") or DEFAULT_PASSWORD
    min_steps = _env_int(("STEP_MIN", "MI_MIN_STEPS"), DEFAULT_MIN_STEPS)
    max_steps = _env_int(("STEP_MAX", "MI_MAX_STEPS"), DEFAULT_MAX_STEPS)

    if not user or not password:
        print("未配置账号！请设置环境变量：")
        print('  set MI_USER=你的账号        （手机号或邮箱，多账号用 # 分隔）')
        print('  set MI_PASSWD=你的密码      （多账号用 # 分隔）')
        print('  set STEP_MIN=8000  set STEP_MAX=20000 （可选，步数范围）')
        sys.exit(1)

    ok, summary = run_accounts(user, password, min_steps, max_steps)
    print(summary)
    sys.exit(0 if ok else 1)


# ======================== 步数数据模板 ========================
# 原始完整模板中的 data_json（URL 编码），仅其中的日期 2025-08-17 与总步数 17760 会在运行时被替换
DATA_JSON_TEMPLATE = "%5B%7B%22data_hr%22%3A%22%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9L%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FVv%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0v%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9e%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0n%5C%2Fa%5C%2F%5C%2F%5C%2FS%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0b%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F1FK%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FR%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9PTFFpaf9L%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FR%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0j%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9K%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FOv%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2Fzf%5C%2F%5C%2F%5C%2F86%5C%2Fzr%5C%2FOv88%5C%2Fzf%5C%2FPf%5C%2F%5C%2F%5C%2F0v%5C%2FS%5C%2F8%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FSf%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2Fz3%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0r%5C%2FOv%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FS%5C%2F9L%5C%2Fzb%5C%2FSf9K%5C%2F0v%5C%2FRf9H%5C%2Fzj%5C%2FSf9K%5C%2F0%5C%2F%5C%2FN%5C%2F%5C%2F%5C%2F%5C%2F0D%5C%2FSf83%5C%2Fzr%5C%2FPf9M%5C%2F0v%5C%2FOv9e%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FS%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2Fzv%5C%2F%5C%2Fz7%5C%2FO%5C%2F83%5C%2Fzv%5C%2FN%5C%2F83%5C%2Fzr%5C%2FN%5C%2F86%5C%2Fz%5C%2F%5C%2FNv83%5C%2Fzn%5C%2FXv84%5C%2Fzr%5C%2FPP84%5C%2Fzj%5C%2FN%5C%2F9e%5C%2Fzr%5C%2FN%5C%2F89%5C%2F03%5C%2FP%5C%2F89%5C%2Fz3%5C%2FQ%5C%2F9N%5C%2F0v%5C%2FTv9C%5C%2F0H%5C%2FOf9D%5C%2Fzz%5C%2FOf88%5C%2Fz%5C%2F%5C%2FPP9A%5C%2Fzr%5C%2FN%5C%2F86%5C%2Fzz%5C%2FNv87%5C%2F0D%5C%2FOv84%5C%2F0v%5C%2FO%5C%2F84%5C%2Fzf%5C%2FMP83%5C%2FzH%5C%2FNv83%5C%2Fzf%5C%2FN%5C%2F84%5C%2Fzf%5C%2FOf82%5C%2Fzf%5C%2FOP83%5C%2Fzb%5C%2FMv81%5C%2FzX%5C%2FR%5C%2F9L%5C%2F0v%5C%2FO%5C%2F9I%5C%2F0T%5C%2FS%5C%2F9A%5C%2Fzn%5C%2FPf89%5C%2Fzn%5C%2FNf9K%5C%2F07%5C%2FN%5C%2F83%5C%2Fzn%5C%2FNv83%5C%2Fzv%5C%2FO%5C%2F9A%5C%2F0H%5C%2FOf8%5C%2F%5C%2Fzj%5C%2FPP83%5C%2Fzj%5C%2FS%5C%2F87%5C%2Fzj%5C%2FNv84%5C%2Fzf%5C%2FOf83%5C%2Fzf%5C%2FOf83%5C%2Fzb%5C%2FNv9L%5C%2Fzj%5C%2FNv82%5C%2Fzb%5C%2FN%5C%2F85%5C%2Fzf%5C%2FN%5C%2F9J%5C%2Fzf%5C%2FNv83%5C%2Fzj%5C%2FNv84%5C%2F0r%5C%2FSv83%5C%2Fzf%5C%2FMP%5C%2F%5C%2F%5C%2Fzb%5C%2FMv82%5C%2Fzb%5C%2FOf85%5C%2Fz7%5C%2FNv8%5C%2F%5C%2F0r%5C%2FS%5C%2F85%5C%2F0H%5C%2FQP9B%5C%2F0D%5C%2FNf89%5C%2Fzj%5C%2FOv83%5C%2Fzv%5C%2FNv8%5C%2F%5C%2F0f%5C%2FSv9O%5C%2F0ZeXv%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F1X%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9B%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2FTP%5C%2F%5C%2F%5C%2F1b%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F0%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F9N%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2F%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%5C%2Fv7%2B%22%2C%22date%22%3A%222025-08-17%22%2C%22data%22%3A%5B%7B%22start%22%3A0%2C%22stop%22%3A1439%2C%22value%22%3A%%22%2C%22tz%22%3A32%2C%22did%22%3A%22DA932FFFFE8816E7%22%2C%22src%22%3A24%7D%5D%2C%22summary%22%3A%22%7B%5C%22v%5C%22%3A6%2C%5C%22slp%5C%22%3A%7B%5C%22st%5C%22%3A1755407692%2C%5C%22ed%5C%22%3A1755407692%2C%5C%22dp%5C%22%3A0%2C%5C%22lt%5C%22%3A0%2C%5C%22wk%5C%22%3A0%2C%5C%22usrSt%5C%22%3A-1440%2C%5C%22usrEd%5C%22%3A-1440%2C%5C%22wc%5C%22%3A0%2C%5C%22is%5C%22%3A0%2C%5C%22lb%5C%22%3A0%2C%5C%22to%5C%22%3A0%2C%5C%22dt%5C%22%3A0%2C%5C%22rhr%5C%22%3A0%2C%5C%22ss%5C%22%3A0%7D%2C%5C%22stp%5C%22%3A%7B%5C%22ttl%5C%22%3A17760%2C%5C%22dis%5C%22%3A10627%2C%5C%22cal%5C%22%3A510%2C%5C%22wk%5C%22%3A41%2C%5C%22rn%5C%22%3A50%2C%5C%22runDist%5C%22%3A7654%2C%5C%22runCal%5C%22%3A397%2C%5C%22stage%5C%22%3A%5B%7B%5C%22start%5C%22%3A327%2C%5C%22stop%5C%22%3A341%2C%5C%22mode%5C%22%3A1%2C%5C%22dis%5C%22%3A481%2C%5C%22cal%5C%22%3A13%2C%5C%22step%5C%22%3A680%7D%2C%7B%5C%22start%5C%22%3A342%2C%5C%22stop%5C%22%3A367%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A2295%2C%5C%22cal%5C%22%3A95%2C%5C%22step%5C%22%3A2874%7D%2C%7B%5C%22start%5C%22%3A368%2C%5C%22stop%5C%22%3A377%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A1592%2C%5C%22cal%5C%22%3A88%2C%5C%22step%5C%22%3A1664%7D%2C%7B%5C%22start%5C%22%3A378%2C%5C%22stop%5C%22%3A386%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A1072%2C%5C%22cal%5C%22%3A51%2C%5C%22step%5C%22%3A1245%7D%2C%7B%5C%22start%5C%22%3A387%2C%5C%22stop%5C%22%3A393%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A1036%2C%5C%22cal%5C%22%3A57%2C%5C%22step%5C%22%3A1124%7D%2C%7B%5C%22start%5C%22%3A394%2C%5C%22stop%5C%22%3A398%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A488%2C%5C%22cal%5C%22%3A19%2C%5C%22step%5C%22%3A607%7D%2C%7B%5C%22start%5C%22%3A399%2C%5C%22stop%5C%22%3A414%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A2220%2C%5C%22cal%5C%22%3A120%2C%5C%22step%5C%22%3A2371%7D%2C%7B%5C%22start%5C%22%3A415%2C%5C%22stop%5C%22%3A427%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A1268%2C%5C%22cal%5C%22%3A59%2C%5C%22step%5C%22%3A1489%7D%2C%7B%5C%22start%5C%22%3A428%2C%5C%22stop%5C%22%3A433%2C%5C%22mode%5C%22%3A1%2C%5C%22dis%5C%22%3A152%2C%5C%22cal%5C%22%3A4%2C%5C%22step%5C%22%3A238%7D%2C%7B%5C%22start%5C%22%3A434%2C%5C%22stop%5C%22%3A444%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A2295%2C%5C%22cal%5C%22%3A95%2C%5C%22step%5C%22%3A2874%7D%2C%7B%5C%22start%5C%22%3A445%2C%5C%22stop%5C%22%3A455%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A1592%2C%5C%22cal%5C%22%3A88%2C%5C%22step%5C%22%3A1664%7D%2C%7B%5C%22start%5C%22%3A456%2C%5C%22stop%5C%22%3A466%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A1072%2C%5C%22cal%5C%22%3A51%2C%5C%22step%5C%22%3A1245%7D%2C%7B%5C%22start%5C%22%3A467%2C%5C%22stop%5C%22%3A477%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A1036%2C%5C%22cal%5C%22%3A57%2C%5C%22step%5C%22%3A1124%7D%2C%7B%5C%22start%5C%22%3A478%2C%5C%22stop%5C%22%3A488%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A488%2C%5C%22cal%5C%22%3A19%2C%5C%22step%5C%22%3A607%7D%2C%7B%5C%22start%5C%22%3A489%2C%5C%22stop%5C%22%3A499%2C%5C%22mode%5C%22%3A4%2C%5C%22dis%5C%22%3A2220%2C%5C%22cal%5C%22%3A120%2C%5C%22step%5C%22%3A2371%7D%2C%7B%5C%22start%5C%22%3A500%2C%5C%22stop%5C%22%3A511%2C%5C%22mode%5C%22%3A3%2C%5C%22dis%5C%22%3A1268%2C%5C%22cal%5C%22%3A59%2C%5C%22step%5C%22%3A1489%7D%2C%7B%5C%22start%5C%22%3A512%2C%5C%22stop%5C%22%3A522%2C%5C%22mode%5C%22%3A1%2C%5C%22dis%5C%22%3A152%2C%5C%22cal%5C%22%3A4%2C%5C%22step%5C%22%3A238%7D%5D%7D%2C%5C%22goal%5C%22%3A8000%2C%5C%22tz%5C%22%3A%5C%2228800%5C%22%7D%22%2C%22source%22%3A24%2C%22type%22%3A0%7D%5D"
