抱歉给你带来的不便！GitHub 的 Markdown 渲染有时会因为格式问题或复制时的空格、换行等导致显示不正确。为了方便你直接复制并在 GitHub 上正常显示，我将提供一个简洁且易于复制的版本，确保格式在 GitHub 上也能正确渲染。同时，我也会说明如何完整复制格式以避免问题。

### 易于复制的版本
以下是优化后的 Markdown 内容，已确保在 GitHub 上可以正常渲染。你可以直接全选复制并粘贴到 GitHub 的 `README.md` 文件中：
